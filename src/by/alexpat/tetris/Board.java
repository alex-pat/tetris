...
public class Board extends StackPane {
     ...
     * This is the area where the tetraminos spawn.
    ...
    /**
     * The list of tetraminos, which are coming next.
     */
    private final ObservableList<Tetramino> waitingTetraminos = FXCollections.observableArrayList();
    ...
    /**
     * The current x and y position with the matrix of the current tetramino.
     */
    private int x = 0, y = 0;

    /**
     * True, while the tetramino is dropped (with the space key).
     */
    ...
    private void spawnTetramino() {
        ...
        while (waitingTetraminos.size() <= MAX_PREVIEWS) {
            waitingTetraminos.add(Tetramino.random(squareSize));
        }
        ...
        translateTransition.setNode(currentTetramino);
        moveDownTransition.setNode(currentTetramino);
        moveDownFastTransition.setNode(currentTetramino);

        // Add the current tetramino to the board.
        getChildren().add(currentTetramino);

        // Move it to the correct position
        // Spawn the tetramino in the middle (I, O) or in the left middle (J, L, S, T, Z).
        x = (matrix[0].length - currentTetramino.getMatrix().length) / 2;
        y = 0;
        // Translate the tetramino to its starting position.
        currentTetramino.setTranslateY((y - Board.HIDDEN_ROWS) * getSquareSize());
    ...
    private void mergeTetraminoWithBoard() {
        ...
        for (int i = 0; i < tetraminoMatrix.length; i++) {
            for (int j = 0; j < tetraminoMatrix[i].length; j++) {
                ...
                if (tetraminoMatrix[i][j] == 1 && y < BLOCKS_PER_COLUMN + HIDDEN_ROWS && x < BLOCKS_PER_ROW) {
                    ...
                    rectangle.setFill(currentTetramino.getFill());
                    ((Light.Distant) currentTetramino.getLighting().getLight()).azimuthProperty().set(225);
                    rectangle.setEffect(currentTetramino.getLighting());
        ...
        for (int i = y + currentTetramino.getMatrix().length - 1; i >= 0; i--) {
            if (i < matrix.length) {
    ...
    /**
     * Clears the board and the waiting tetraminos.
     */
    public void clear() {
    ...
    public void dropDown() {
        ...
        while (!intersectsWithBoard(currentTetramino.getMatrix(), x, y));
        y--;
        isDropping = true;
        dropDownTransition.setNode(currentTetramino);
    ...
    public boolean rotate(final HorizontalDirection direction) {
        ...
        if (currentTetramino == null) {
            ...
            int[][] matrix = currentTetramino.getMatrix();
            ...
            if (!intersectsWithBoard(newMatrix, x, y)) {
                currentTetramino.setMatrix(newMatrix);
                ...
                KeyValue kv = new KeyValue(((Light.Distant) currentTetramino.getLighting().getLight()).azimuthProperty(), 360 - 225 + 90 - rotateTransition.getToAngle());
    ...
    /**
     * Moves the tetramino to left or right.
     *
     * @param direction The horizontal direction.
    ...
    public boolean move(final HorizontalDirection direction) {
        boolean result;
        if (currentTetramino == null || isDropping) {
            result = false;
        } else {
            ...
            if (!moving && !intersectsWithBoard(currentTetramino.getMatrix(), x, y) || moving && !intersectsWithBoard(currentTetramino.getMatrix(), x, y) && !intersectsWithBoard(currentTetramino.getMatrix(), x, y + 1)) {
    ...
    /**
     * Moves the tetramino one field down.
     */
    public void moveDown() {
        if (!isDropping && currentTetramino != null) {
            moveDownFastTransition.stop();
            moving = true;

            // If it is able to move to the next y position, do it!
            if (!intersectsWithBoard(currentTetramino.getMatrix(), x, y + 1) && !isDropping) {
    ...
    /**
     * Moves the current tetramino down fast, if it not already dropping.
     */
    public void moveDownFast() {
        if (!isDropping) {
            ...
            if (!intersectsWithBoard(currentTetramino.getMatrix(), x, y + 1)) {
     ...
     */
    public ObservableList<Tetramino> getWaitingTetraminos() {
        return waitingTetraminos;
    }
    ...
    public static interface BoardListener extends EventListener {
        ...
        /**
         * Called, when a tetramino is dropped or a complete row is dropped after some rows were eliminated.
         */
        void onDropped();
